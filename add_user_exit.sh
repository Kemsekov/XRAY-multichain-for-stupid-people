#!/bin/bash
# ============================================================
# add_user_exit.sh - Add a new user directly to the Xray exit server
# Usage: ./add_user_exit.sh <exit_ip> <exit_user> <exit_pass> <user_name> [sni]
# ============================================================
...
if [ $# -lt 4 ] || [ $# -gt 5 ]; then
    ...
    echo "Usage: $0 <exit_ip> <exit_user> <exit_pass> <user_name> [sni]"
    exit 1
fi
...
ssh_cmd() {
    sshpass -p "$EXIT_PASS" ssh -o StrictHostKeyChecking=no "$EXIT_USER@$EXIT_IP" "$1"
}

echo "============================================"
echo "Adding a new user directly to exit server ($EXIT_IP)"
echo "============================================"
...
if [ -z "$EXIT_PUB" ] || [ -z "$EXIT_SHORT" ]; then
    echo "Error: Could not retrieve public key or short ID."
    exit 1
fi

# Generate VLESS links
VLESS_LINK_TCP="vless://${NEW_UUID}@${EXIT_IP}:443?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${USE_SNI}&fp=chrome&pbk=${EXIT_PUB}&sid=${EXIT_SHORT}#${USERNAME}_Direct_TCP"
VLESS_LINK_GRPC="vless://${NEW_UUID}@${EXIT_IP}:8443?encryption=none&security=reality&sni=${USE_SNI}&fp=chrome&pbk=${EXIT_PUB}&sid=${EXIT_SHORT}&type=grpc&serviceName=xray_grpc_service&mode=gun#${USERNAME}_Direct_gRPC"

echo ""
echo "============================================"
echo "✅ New user added directly to exit server!"
echo "============================================"
echo "Connection strings (Direct to Exit):"
echo ""
...
echo "  Address:    $EXIT_IP"
echo "  UUID:       $NEW_UUID"
echo "  SNI:        $USE_SNI"
echo "  Public Key: $EXIT_PUB"
echo "  Short ID:   $EXIT_SHORT"
echo "============================================"