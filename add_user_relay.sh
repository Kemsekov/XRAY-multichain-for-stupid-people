...
if [ ! -f "sni_relay.json" ]; then
    echo "Error: sni_relay.json not found. Please create it with allowed SNI values."
    exit 1
fi

# Read and parse allowed SNIs from sni.json
ALLOWED_SNIS=$(jq -r '.[]' sni_relay.json)
FIRST_SNI=$(jq -r '.[0]' sni_relay.json)

if [ -z "$FIRST_SNI" ] || [ "$FIRST_SNI" = "null" ]; then
    echo "Error: sni_relay.json is empty or invalid."
    exit 1
fi
